            ...
            .map((_, i) => (
              <div
                ...
                style={{
                  ...
                  height: 5,
                  borderRadius: 5,
                  background: i < step ? C.green : "#E9EDE9",
...
function StepRCInput({ onNext, savedData, onSave }) {
  ...
  const [plateDisplay, setPlateDisplay] = useState(
    savedData?.rcNum ? formatPlate(savedData.rcNum) : "MH 01 AB 1234",
  );
  const [frontPreview, setFrontPreview] = useState(
    savedData?.rcFrontPreview || null,
  );
  const [backPreview, setBackPreview] = useState(
    savedData?.rcBackPreview || null,
  );
  ...
  const submit = async () => {
    if (rcNum.length < 5) {
      setError("Please enter a valid RC number (min 5 chars)");
      return;
    }
    if (!frontFile || !backFile) {
      setError("Please upload both RC front and back images");
      return;
    }
    ...
    <div style={{ animation: "fadeUp 0.32s ease" }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <input
          ...
          accept="image/*"
          style={{ display: "none" }}
          onChange={(e) => handleImagePick("front", e)}
        />
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <input
          ...
          accept="image/*"
          style={{ display: "none" }}
          onChange={(e) => handleImagePick("back", e)}
        />
      </div>
    ...
    <>
      ...
      <div style={{ animation: "fadeUp 0.32s ease" }}>
        ...
        <div className="section-card">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              onSave({ km: Number(e.target.value) });
            }}
            style={{ width: "100%" }}
          />
          <div
        ...
        <div className="section-card">
          ...
          <ToggleRow
            label="Scratches or Dents?"
            sub="Visible damage on the car"
            checked={scratches}
            onChange={(v) => {
              setScratch(v);
              onSave({ scratches: v });
          ...
          <ToggleRow
            ...
            sub="Any major collision history"
            checked={accidents}
            onChange={(v) => {
              setAccid(v);
              onSave({ accidents: v });
          ...
          <ToggleRow
            ...
            sub="Is the car currently drivable"
            checked={isRunning}
            onChange={(v) => {
              setRunning(v);
              onSave({ isRunning: v });
          ...
          />
          <ToggleRow
            label="Any Missing Parts?"
            sub="Parts removed or not installed"
            checked={missingPart}
            onChange={(v) => {
              setMissing(v);
              onSave({ missingPart: v });
            }}
          />
        </div>
        ...
        <div className="section-card">
          ...
          <div style={{ fontSize: 11, color: C.textSoft, marginBottom: 14 }}>
            Front photo is required. Others recommended.
          </div>
          <div
            style={{
              display: "grid",
  ...
  const breakdown = [
    {
      ...
      value: `₹${p.basePrice.toLocaleString("en-IN")}`,
      type: "normal",
    },
    {
      label: `Year Depreciation (${p.age} yrs)`,
      value: `−₹${p.yearDepreciation.toLocaleString("en-IN")}`,
      type: "neg",
    },
    {
      label: `KM Driven (${p.kmDriven?.toLocaleString("en-IN")})`,
      value: `−₹${p.kmDepreciation.toLocaleString("en-IN")}`,
      type: "neg",
    },
    {
      label: "1st Owner Bonus",
      value:
        p.ownerBonus > 0 ? `+₹${p.ownerBonus.toLocaleString("en-IN")}` : "₹0",
      type: p.ownerBonus > 0 ? "pos" : "normal",
    },
    {
      label: "Market Demand",
      value: `+₹${p.marketDemand.toLocaleString("en-IN")}`,
      type: "pos",
    },
    ...(p.notRunningPenalty > 0
      ? [
          {
            label: "Not Running Condition",
            value: `−₹${p.notRunningPenalty.toLocaleString("en-IN")}`,
            type: "neg",
          },
        ]
      : []),
    ...(p.missingPartPenalty > 0
      ? [
          {
            label: "Missing Parts Penalty",
            value: `−₹${p.missingPartPenalty.toLocaleString("en-IN")}`,
            type: "neg",
          },
        ]
      : []),
  ...
  return (
    <>
      ...
      <div style={{ animation: "fadeUp 0.32s ease" }}>
        ...
        <div
          style={{
            background: `linear-gradient(145deg, #0A2E17 0%, ${C.greenDark} 50%, #1356CC 100%)`,
            borderRadius: 20,
            padding: "32px 24px",
            textAlign: "center",
            marginBottom: 16,
        ...
        <div className="section-card">
          ...
          {breakdown.map((b, i) => (
            <div key={i} className="breakdown-row">
              ...
              <span
                className={`breakdown-val ${b.type === "pos" ? "breakdown-pos" : b.type === "neg" ? "breakdown-neg" : ""}`}
              >
                {b.value}
              </span>
            </div>
        ...
        >
          {[
            ...
            >
              <div
                style={{ fontSize: 17, fontWeight: 900, color: C.greenDark }}
              >
                {h}
              </div>
              <div
    ...
    <div style={{ textAlign: "center", animation: "fadeUp 0.4s ease" }}>
      <div
        style={{
          ...
          borderRadius: "50%",
          background: C.greenLight,
          border: `3px solid ${C.greenMid}`,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          margin: "0 auto 24px",
          animation: "scaleIn 0.5s cubic-bezier(.3,1.4,.5,1)",
        }}
      >
      ...
      >
        Your vehicle request has been successfully submitted. Our team member
        ...
        {details.map(([k, v], i) => (
          <div
            ...
            style={{
              ...
              padding: "10px 0",
              borderBottom:
                i < details.length - 1 ? `1px solid ${C.border}` : "none",
              gap: 12,
            }}
          >
      ...
      <button className="btn-ghost" onClick={onGoListing}>
        View My Listings
      </button>
      <div style={{ fontSize: 11, color: C.textSoft, marginTop: 20 }}>
        Bharat Scrap v1.0.0 · Made with ❤️ in India
      </div>
    </div>
  );
}
...
export default function CarListingPage() {
  ...
  const shell = (children, canBack) => (
    ...
    >
      ...
      />
      <div
        style={{ maxWidth: 520, margin: "0 auto", padding: "20px 16px 48px" }}
      >
        {children}
      </div>
    </div>
  ...
  if (step === 3)
    return shell(
      <StepConditionForm
        ...
        onBack={() => goToStep(2)}
        onNext={(updatedCar) => {
          saveField({ carData: { ...progress.carData, ...updatedCar } });
          goToStep(4);
        }}
      />,
      true,

...
function InfoRow({ label, value }) {
  ...
  return (
    <div className="history-info-row">
      ...
      <span className="history-info-value">{String(value)}</span>
    </div>
  );
}
  ...
  return (
    <div className="history-person-card">
      ...
      <div className="history-person-info">
        ...
        {person.phone && (
          <div className="history-person-row">
...
export default function Page() {
  ...
  const fetchCarDetail = async () => {
    ...
    try {
      ...
      const data = res.data;
      if (data.success) setCarDetail(data.data);
      else setError(true);
    } catch {
      setError(true);
    } finally {
  ...
  const {
    ...
    tyreImage, odometerImage, rcFrontImage, price, rcBackImage,
    createdAt, updatedAt,
  } = carDetail;
        ...
        <div className="history-scroll">
          ...
          <div className="history-hero">
            ...
            <div className="history-hero-content">
              ...
              <div className="history-hero-stats">
                ...
                {kmDriven != null && (
                  <div className="history-stat-pill">
                    <span className="history-stat-num">{Number(kmDriven).toLocaleString()}</span>
            ...
            {(frontImage?.image || (images && images[0]?.image)) && (
              <div className="history-hero-img-wrap">
                <img
                  src={frontImage?.image || images[0]?.image}
                  alt="Car"
          ...
          <div className="history-panels">
            ...
            {activeTab === 'vehicle' && rc && (
              <Section title="Vehicle Information" icon={<CarIcon color={ACCENT} />} delay={0}>
            ...
            {activeTab === 'registration' && rc && (
              <Section title="Registration Details" icon={<LocationIcon color={ACCENT} />} delay={0}>
            ...
            {activeTab === 'condition' && (
              <Section title="Condition Report" icon={<ShieldIcon color={ACCENT} />} delay={0}>
                ...
                {kmDriven != null && <InfoRow label="KM Driven" value={`${Number(kmDriven).toLocaleString()} km`} />}
                {pickupLocation && <InfoRow label="Pickup Location" value={pickupLocation} />}
              </Section>
            )}

            {activeTab === 'payment' && (
              <Section title="Payment Details" icon={<PaymentIcon color={ACCENT} />} delay={0}>
                <BoolRow label="Payment Done" value={isPaid} />
                {paymentMethod && <InfoRow label="Method" value={paymentMethod === 'upi' ? 'UPI' : 'Bank Transfer'} />}
                {price && <InfoRow label="Price" value={`₹ ${Number(price).toLocaleString()}`} />}
          ...
          <div className="history-people-grid">
            {seller && (
              <div className="history-people-card">
                <div className="history-people-label">
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .history-hero-bg {
    ...
    inset: 0;
    background:
      radial-gradient(ellipse 60% 80% at 90% 50%, rgba(61,186,94,0.18) 0%, transparent 70%),
      radial-gradient(ellipse 40% 40% at 10% 80%, rgba(61,186,94,0.08) 0%, transparent 60%);
    pointer-events: none;
  }
  .history-hero-content { position: relative; z-index: 2; max-width: 60%; }
  ...
  .history-tag {
    padding: 3px 9px;
    border-radius: 20px;
    background: rgba(61,186,94,0.18);
    color: #6ee89a;
    font-size: 10px;
    font-weight: 600;
    text-transform: uppercase;
    letter-spacing: 0.5px;
    border: 1px solid rgba(61,186,94,0.25);
  }
  .history-hero-title {
    font-family: 'Cabinet Grotesk', sans-serif;
    font-size: 26px;
  ...
  .history-section {
    ...
    box-shadow: 0 2px 8px rgba(10,31,13,0.04);
  }
  @keyframes h-fadeup {
  ...
  .history-section-header {
    ...
    padding: 14px 16px;
    border-bottom: 1px solid var(--h-border);
    background: linear-gradient(to right, rgba(61,186,94,0.04), transparent);
  }
  ...
  .history-info-row {
    ...
    align-items: center;
    padding: 9px 0;
    border-bottom: 1px solid rgba(10,31,13,0.04);
    gap: 12px;
  }
  ...
  .history-person-avatar {
    ...
    border-radius: 23px;
    background: var(--h-accent-dim);
    border: 2px solid rgba(61,186,94,0.2);
    overflow: hidden;
    flex-shrink: 0;
    display: flex;
    align-items: center;
  ...
  .history-people-card {
    ...
    border: 1px solid var(--h-border);
    padding: 14px;
    box-shadow: 0 1px 4px rgba(10,31,13,0.04);
  }
  .history-people-label {
    display: flex;
    align-items: center;
    gap: 6px;
    font-family: 'Cabinet Grotesk', sans-serif;
  ...
  .history-timeline-dot {
    ...
    box-shadow: 0 0 0 2px #d4ddd4;
    flex-shrink: 0;
    margin-top: 3px;
  }
  .history-timeline-dot-accent {
    background: var(--h-accent);
    box-shadow: 0 0 0 2px rgba(61,186,94,0.25);
  }

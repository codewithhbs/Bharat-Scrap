...
import {
  Search, Eye, Trash2, X, MessageSquare,
  Phone, Mail, User, ChevronLeft, ChevronRight, Recycle,
} from "lucide-react";
...
export default function AllContactMessages() {
  ...
  const confirmDelete = (msg) => {
    Swal.fire({
      ...
      text: `This will permanently delete the message from ${msg.name || "this user"}. Cannot be undone.`,
  ...
  const viewMessage = (msg) => {
    Swal.fire({
      title: `Message from ${msg.name || "Unknown"}`,
      html: `
        <div style="text-align:left; font-size:14px; color:#374151; line-height:1.6;">
          <p style="margin:0 0 6px"><strong>Email:</strong> ${msg.email || "—"}</p>
          <p style="margin:0 0 6px"><strong>Phone:</strong> ${msg.phone || "—"}</p>
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
      ...
      {/* ── Stats ── */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 14, marginBottom: 24, maxWidth: 420 }}>
        {[
          { label: "Total Messages", value: total,          icon: MessageSquare, color: "#166534", bg: "#dcfce7" },
          { label: "Showing",        value: messages.length, icon: Eye,           color: "#065f46", bg: "#d1fae5" },
          { label: "Pages",          value: totalPages,      icon: ChevronRight,  color: "#374151", bg: "#f3f4f6" },
        ].map((s, i) => (
      ...
      {!loading && messages.length === 0 ? (
        <div style={{ background: "#fff", border: "1.5px solid #e7f3e8", borderRadius: 16,
          ...
          <p style={{ margin: 0, fontSize: 15, color: "#6b7280", fontWeight: 500 }}>No messages found</p>
          <p style={{ margin: "4px 0 12px", fontSize: 13, color: "#9ca3af" }}>
            {searchInput ? "Try adjusting your search" : "No contact messages yet"}
          </p>
          {searchInput && (
            <button onClick={() => setSearchInput("")} style={{ padding: "8px 18px", borderRadius: 9,
              border: "1.5px solid #e5e7eb", background: "#fff", color: "#374151",
              fontSize: 13, cursor: "pointer" }}>Clear Search</button>
          )}
        </div>
      ) : (
        <>
                    ...
                    >
                      ...
                      <td style={{ padding: "13px 16px", whiteSpace: "nowrap" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                          <div style={{
                            width: 34, height: 34, borderRadius: 9, background: "#dcfce7",
                            display: "flex", alignItems: "center", justifyContent: "center",
                            flexShrink: 0,
                          }}>
          ...
          <div style={{ marginTop: 20, display: "flex", justifyContent: "space-between",
            ...
            <p style={{ margin: 0, fontSize: 13, color: "#6b7280" }}>
              ...
              {" of "}
              <strong style={{ color: "#111827" }}>{total}</strong> messages
            </p>
